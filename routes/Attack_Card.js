...
router.get('/', async (req, res, next) => {
  ...
  const data = await req.truyvan('SELECT * FROM Attack_Card')
  const header = await req.truyvan("SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'Attack_Card'")
  const headers = header.map(row => row.COLUMN_NAME);
  const imageLinks = req.imageLinksAT;
  ...
  //   }
    ...
    try {
      ...
      //   const data1 = rows.slice(1).filter(row => row[headers.indexOf('Name')] && row.length > 0).map((row, index) => {
      //       let item = {};
      ...
      res.render('Attack_Card', { dulieu, headers, categories });
    } catch (error) {
